...
def perfect_align(t, T, insertions_allowed=0,
    ...
    # The only known starting point. Nothing matched to nothing.
    dp[0, 0, 0] = 0
    for i in range(len(t) + 1):
        for j in range(len(T) + 1):
            for q in range(insertions_allowed + 1):
                if i < len(t):
...
# align source-target pairs，return edit seq
...
def convert_data_from_raw_files(source_file, target_file, output_file, chunk_size):
    ...
    # all sents, successfully converted，sents with grammar erros
...
def main(args):
    convert_data_from_raw_files(
        args.source, args.target, args.output_file, args.chunk_size)
